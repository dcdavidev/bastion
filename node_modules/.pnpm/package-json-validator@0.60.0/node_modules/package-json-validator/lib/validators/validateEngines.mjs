...
//#region src/validators/validateEngines.ts
/**
...
const validateEngines = (obj) => {
	...
	if (obj && typeof obj === "object" && !Array.isArray(obj)) {
		...
		for (const [key, value] of Object.entries(obj)) {
			...
			else if (value.trim() === "") childResult.addIssue(`the value of property ${fieldName} is empty, but should be a semver range`);
			if (key.trim() === "") childResult.addIssue(`property ${fieldName} has an empty key, but should be a runtime or package manager`);
			result.addChildResult(childResult);
			propertyNumber++;
		}
	} else if (obj === null) result.addIssue("the value is `null`, but should be an `object`");
	else {
