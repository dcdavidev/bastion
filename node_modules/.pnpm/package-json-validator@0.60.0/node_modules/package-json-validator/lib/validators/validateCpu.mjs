...
//#region src/validators/validateCpu.ts
const VALID_ARCHS = [
	"arm",
	"arm64",
	"ia32",
...
const validateCpu = (obj) => {
	...
	if (Array.isArray(obj)) for (let i = 0; i < obj.length; i++) {
		...
		} else if (item.trim() === "") childResult.addIssue(`item at index ${i} is empty, but should be the name of a CPU architecture`);
		else if (!VALID_ARCHS.includes(item)) childResult.addIssue(`the value "${item}" is not valid. Valid CPU values are: ${VALID_ARCHS.join(", ")}`);
